...
int main(int count, char* input_parameters[])
{
  ...
  try{
    ...
    string command;
    command = input_parameters[1];

    if(command == "weblink_genkey"){
      string secretkeypath = input_parameters[2];
      string publickeypath = input_parameters[3];
      string relinkeypath = input_parameters[4];
    ...
    else if(command == "weblink_encrypt"){
      string pubkeypath = input_parameters[2];
      string serverfilename = input_parameters[3];
      string fileout = input_parameters[4];
      string pathout = input_parameters[5];

      system(("rm -rf "+ pathout + "/*").c_str());

      encryptfiles(pubkeypath, serverfilename, fileout + "/", pathout);
    }
    else if(command == "weblink_encryptweightcsv"){
      string pubkeypath = input_parameters[2];
      string serverfilename = input_parameters[3];
      string numlength_str = input_parameters[4];
      string fileout = input_parameters[5];
      string pathout = input_parameters[6];

      system(("rm -rf "+ pathout + "/*").c_str());

      encryptweightfiles(pubkeypath, serverfilename, numlength_str, fileout + "/", pathout);
    }
    else if(command == "weblink_encryptvotecsv"){
      string pubkeypath = input_parameters[2];
      string clientfilename = input_parameters[3];
      string fileout = input_parameters[4];
      string pathout = input_parameters[5];
    ...
    else if(command == "weblink_encryptvote"){
      string pubkeypath = input_parameters[2];
      string name = input_parameters[3];
      string vote = input_parameters[4];
      string fileout = input_parameters[5];
      
      string fileout_path = fileout + "/";
      string identifier,votename;
      encryptvotefile(pubkeypath, name, vote, fileout_path, identifier, votename);

      string filename = fileout_path + votename;

      cout<<name<<","<<identifier<<","<<votename;
    }
    else if(command == "weblink_encryptweight"){
      string pubkeypath = input_parameters[2];
      string numlength_str = input_parameters[3];
      string name = input_parameters[4];
      string weight = input_parameters[5];
      string fileout = input_parameters[6];

      string identifier, weightname;
      string fileout_path = fileout + "/";
      encryptweightfile(pubkeypath, name, weight, numlength_str, fileout_path, identifier, weightname);

      string filename =  fileout_path + weightname;
      
      cout<<name<<","<<identifier<<","<<weightname;
    }
    else if(command == "weblink_FHE_operation_result"){
      string relinkey_path = input_parameters[2];
      string server_str = input_parameters[3];
      string client_str = input_parameters[4];
      // string encryptfile1_path = input_parameters[5];
      // string encryptfile2_path = input_parameters[6];
      string numlength_str = input_parameters[5];
    ...
    else if(command == "weblink_insert_to_FHE_operation"){
      string relinkey_path = input_parameters[2];
      string insertfile_path = input_parameters[3];
      string weight_path = input_parameters[4];
      string vote_path = input_parameters[5];
      string result_path = input_parameters[6];

      insert_to_FHE_operation(relinkey_path, insertfile_path, weight_path, vote_path, result_path);
    }
    else if(command == "weblink_eval"){
      string relinkey_path = input_parameters[2];
      string func = input_parameters[3];
      string filein_path = input_parameters[4];
      string filecipher_path = input_parameters[5];
      string result_path = input_parameters[6];

      multiplyoradd(relinkey_path, func, filein_path, filecipher_path, result_path);
    }
    else if(command == "decrypt"){
      string cipher_str = input_parameters[2];
    ...
    else if(command == "weblink_decrypt"){
      string secretkey_path = input_parameters[2];
      string cipher_path = input_parameters[3];
      string result_path = input_parameters[4];
      string candidate = input_parameters[5];
    ...
    ofstream writeexception;
    time_t now = time(0);
    tm *ltm = localtime(&now);
	
    char str[200];
    sprintf(str, "%d_%d_%d-%d_%d_%d-log.txt", (1900 + ltm->tm_year), (1 + ltm->tm_mon), ltm->tm_mday, ltm->tm_hour, ltm->tm_min, ltm->tm_sec);
    string filename = str;
    writeexception.open(sealMainInQueryPath + "native/webserver-encrypt/exceptionlog/" + filename,ios::out|ios::trunc);
    writeexception << "SEAL-main/native/webserver-encrypt/exception.cpp" << endl;
    writeexception << "Exception happened, the detailed information as follows:" << endl;
    writeexception << e.msg << endl;
    ...
    string filename = str;
    writeexception.open(sealMainInQueryPath + "native/webserver-encrypt/exceptionlog/" + filename,ios::out|ios::trunc);
    writeexception << "Exception happened!File:SEAL-main/native/webserver-encrypt/exception.cpp!" << endl;
    writeexception.close();
    exit(1);
  }
}