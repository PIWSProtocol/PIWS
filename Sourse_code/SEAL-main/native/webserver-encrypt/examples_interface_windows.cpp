...
void All_FHE_operation_result(string relinkey_path, string weight_path, string vote_path)
{
  ...
  configure_file.open("C:/config.txt");
  string configs[17];
  string config_line;
  int location = 0;
  ...
  system(("scp "+ relinkey_path + " "+gsUser+"@"+gsIp+":"+sealServerIngsPath+"native/server-add/input/outputserver/"+relienkeyname).c_str());

  system(("scp "+ weight_path +" "+gsUser+"@"+gsIp+":"+sealServerIngsPath+"native/server-add/input/outputserver").c_str());
  system(("scp "+ vote_path +" "+ gcUser +"@"+ gcIp +":"+ priAndjoinClientIngcPath +"clientinput").c_str());
  system(("ssh "+ gcUser +"@"+ gcIp +" "+ priAndjoinClientIngcPath +"private-join-and-compute/bazel-bin/client --client_data_file="+ priAndjoinClientIngcPath +"clientinput/"+ vote_path.substr(vote_path.find_last_of('/')+1) +" > log.txt").c_str());
...
void All_FHE_operation(string vote_path)
{
  ...
  system(("ssh "+ gcUser +"@"+ gcIp +" "+ priAndjoinClientIngcPath +"private-join-and-compute/bazel-bin/client --client_data_file="+ vote_path).c_str());
  // cout<<vote_path.substr(vote_path.find_last_of('/')+1)<<endl;
  ...
  system(("scp "+ gsUser +"@"+ gsIp +":"+ sealServerIngsPath +"native/server-add/input/"+ addedvotelist_csv + " " + sealMainInQueryPath + "native/webserver-encrypt/inputcipher").c_str());
  system(("scp "+ gsUser +"@"+ gsIp +":"+ sealServerIngsPath +"native/server-add/output/"+ resultname + " " + sealMainInQueryPath + "native/webserver-encrypt/inputcipher").c_str());
}
  ...
  (JNIEnv *env, jobject obejct, jstring secretkeypath_jstring, jstring publickeypath_jstring, jstring relinkeypath_jstring)
{
  ...
  string config_file_path = "C:/config.txt";
